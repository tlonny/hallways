...
use crate::hallways::game::state::menu::home::Item;
use crate::hallways::game::state::menu::Home;
...
use crate::hallways::sprite::text::option::State;
use crate::hallways::sprite::text::Option as TextOption;
use crate::hallways::sprite::Border;
...
pub fn update(
    ...
    state_menu_home: &mut Home,
    keyboard: &Keyboard,
    select_speaker: &Speaker,
    move_speaker: &Speaker,
) {
    if !matches!(state_scene.scene(), Kind::MenuHome) {
        return;
    }

    if state_scene.transitioned() {
        state_menu_home.clear();
    }

    if keyboard.pressed(KeyCode::ArrowUp, false) {
        state_menu_home.selected = (state_menu_home.selected + Item::COUNT - 1) % Item::COUNT;
        move_speaker.reset();
        move_speaker.play();
    } else if keyboard.pressed(KeyCode::ArrowDown, false) {
        state_menu_home.selected = (state_menu_home.selected + 1) % Item::COUNT;
        move_speaker.reset();
        move_speaker.play();
    } else if keyboard.pressed(KeyCode::Enter, false) {
        select_speaker.reset();
        select_speaker.play();
        if let Some(item) = Item::iter().nth(state_menu_home.selected) {
            select_item(item, state_scene);
        }
    }
    ...
    for (i, item) in Item::iter().enumerate() {
        ...
        let hovered = i == state_menu_home.selected;
        TextOption::new(
...
fn select_item(item: Item, state_scene: &mut Scene) {
    match item {
        Item::Visit => state_scene.set_scene(Kind::MenuVisit),
        Item::Settings => state_scene.set_scene(Kind::MenuSettings),
        Item::Quit => state_scene.set_scene(Kind::Quit),
    }
}
